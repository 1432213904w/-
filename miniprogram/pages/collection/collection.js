...
Page({
  ...
  onQueryQuestion: function(){
    ...
    wx.cloud.callFunction({
      ...
      complete: res => {
        console.log('callFunction fetchNotes result: ', res);
        let list = res.result.list;
        let arr = [];
        let items = [];
        list.forEach(item=>{
          if(items.indexOf(item.qid) == -1){
            items.push(item.qid);
        ...
        that.setData({
          ...
          console.log('已赋值完成')
          console.log('正确答案')
          console.log(this.data.question['answer'],'009');
          wx.hideLoading()
        })
      },
    })
  ...
  addRecords: function(){
    ...
    let {name, tel} = app.globalData.userInfo;
  ...
  delete: function(){
    ...
    wx.cloud.callFunction({
      ...
      complete: res => {
        console.log('callFunction delCollection result: ', res);
        wx.showToast({
          title: '删除成功',
          icon: 'success',
          duration: 2000
